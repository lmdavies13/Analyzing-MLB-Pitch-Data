...
## 3.5 Investigating 4-1 and 4-2 Pitch Counts

Some x-axis visualizations show pitch counts of 4 balls and 1 or 2 strikes. Since an at-bat should end when the batter receives 4 balls, this raises the question of whether those counts reflect valid pitch sequences (e.g., 0-2 followed by 4 balls) or data issues. We investigate by checking pitch-by-pitch progression in these at-bats. The code below verifies the validity of the 4-1 and 4-2 pitch counts.

```{r}
# Clone the full dataset before cleaning (so ab_id is preserved)
pitch_full <- read_csv("pitches.csv", show_col_types = FALSE)

# Filter pitches where the count is 4 balls and 1 or 2 strikes
suspect_pitches <- pitch_full %>%
  filter(b_count == 4, s_count %in% c(1, 2))

# Get the relevant at-bat IDs
suspect_ids <- unique(suspect_pitches$ab_id)

# Retrieve all pitches from those at-bats
sequences <- pitch_full %>%
  filter(ab_id %in% suspect_ids) %>%
  arrange(ab_id, pitch_num) %>%
  group_by(ab_id) %>%
  mutate(
    prev_b = lag(b_count, default = 0),
    prev_s = lag(s_count, default = 0)
  ) %>%
  ungroup()

# Identify invalid transitions:
# - More balls after reaching 4
# - More strikes after reaching 3
balls_after_walk <- sequences %>%
  filter(prev_b >= 4 & b_count > prev_b)

strikes_after_strikeout <- sequences %>%
  filter(prev_s >= 3 & s_count > prev_s)

invalid <- bind_rows(balls_after_walk, strikes_after_strikeout)

# Confirm validity
if (nrow(invalid) == 0) {
  print("All 4-1 and 4-2 pitch counts are valid — no data issues detected.")
} else {
  print("Some count transitions appear invalid:")
  print(invalid)
}
...
```{r}
#pull out the variables we want for the model
model_vars <- c(
  "nasty","travel_time",
  "spin_rate","spin_dir","break_angle","break_length",
  "vx0","vy0","vz0","pfx_x","pfx_z","x0","z0",
  "b_count","s_count","outs","zone",
...
#create our train test split of 80/20
set.seed(42)
train_flag <- runif(nrow(df_mod)) < .8
train_df   <- df_mod[train_flag, ]
test_df    <- df_mod[!train_flag, ]


# helper to build X-matrix quickly, turns df into numeric design
make_X <- function(data, y) {
  model.matrix(reformulate(setdiff(colnames(data), y), response = NULL), data)[ , -1]
}
```

## 6.2 Linear Regression - Predict Start Speed

```{r}
# make X df
X_train <- make_X(train_df, "start_speed")
X_test  <- make_X(test_df,  "start_speed")

#predict
lm_mod  <- lm(start_speed ~ ., data = train_df)
lm_pred <- predict(lm_mod, newdata = test_df)

#store results for later
lm_rmse <- sqrt(mean((lm_pred - test_df$start_speed)^2))
lm_mae  <- mean(abs(lm_pred - test_df$start_speed))
...
# we need to work on a subset since our dataset is too big, we will do 200,000
sample_rows <- sample(seq_len(nrow(X_train)), 200000) 

#set up rf reg, default params
rf_reg <- randomForest(
  x        = X_train[sample_rows, ],
  y        = train_df$start_speed[sample_rows],
  ntree    = 200,                              
  mtry     = floor(sqrt(ncol(X_train))),
  sampsize = 50000                             
)

#predict and store results for later
rf_pred <- predict(rf_reg, X_test)
rf_rmse <- sqrt(mean((rf_pred - test_df$start_speed)^2))
rf_mae  <- mean(abs(rf_pred - test_df$start_speed))

```

## 6.4 Pitch-type classification

```{r}
library(randomForest)
set.seed(42)

# subset of dataset with equal pull from predictor class
train_sub <- train_df %>%                  
  group_by(pitch_type) %>% 
  sample_n(size = pmin(15000, n()),        
           replace = FALSE) %>%
  ungroup()                                

# set up classification model, default params
rf_cls <- randomForest(
  pitch_type ~ .,
  data      = train_sub,
  ntree     = 200,
  mtry      = floor(sqrt(ncol(train_sub) - 2)),
  sampsize  = 5000          
)

#predict
cls_pred <- predict(rf_cls, test_df)

#store results
acc      <- mean(cls_pred == test_df$pitch_type)
conf_mat <- table(Predicted = cls_pred,
                  Actual    = test_df$pitch_type)

cat("Classification accuracy (sampled RF):", round(acc, 3), "\n")
print(conf_mat)
...
cat("Regression metrics\n",
    "  Linear  : RMSE =", round(lm_rmse,2),  " MAE =", round(lm_mae,2), "\n",
    "  RF      : RMSE =", round(rf_rmse,2),  " MAE =", round(rf_mae,2), "\n\n")

cat("Classification accuracy (Random Forest):", round(acc,3), "\n")
print(conf_mat)
```

The forest reduces regression MAE by 36 % relative to a linear baseline, indicating clear non-linear effects among spin, movement, and release metrics.

For classification, a memory-capped forest trained on \~250 K stratified pitches achieves 68 % accuracy; confusion shows most errors arise from visually similar pitch families (cutter vs four-seam, sinker vs two-seam). These results set our Week 4 baselines
...
# confusion matrix
conf_matrix <- confusionMatrix(cls_pred, test_df$pitch_type)
print(conf_matrix)

# get metrics
eval_df <- data.frame(
  truth = test_df$pitch_type,
  prediction = cls_pred
)

# evaluation metrics
accuracy_val  <- accuracy(eval_df, truth = truth, estimate = prediction)
precision_val <- precision(eval_df, truth = truth, estimate = prediction, estimator = "macro")
recall_val    <- recall(eval_df, truth = truth, estimate = prediction, estimator = "macro")
f1_val        <- f_meas(eval_df, truth = truth, estimate = prediction, estimator = "macro")

# display all 
bind_rows(accuracy_val, precision_val, recall_val, f1_val)

```

```{r}
# convert raw confusion matrix to data frame
cf_df <- as.data.frame(conf_mat)
colnames(cf_df) <- c("Predicted", "Actual", "Freq")

# plot heatmap
ggplot(cf_df, aes(x = Actual, y = Predicted, fill = Freq)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "white", high = "red") +
  labs(title = "Confusion Matrix - Pitch Type Classification",
       x = "Actual Pitch Type",
       y = "Predicted Pitch Type",
       fill = "Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Predict on test set
test_df$predicted <- predict(lm_mod, newdata = test_df)

# Plot
ggplot(test_df, aes(x = predicted, y = start_speed)) +
  geom_point(alpha = 0.5) +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  labs(title = "Predicted vs Actual Pitch Speed",
       x = "Predicted",
       y = "Actual")
...
```{r}
test_df$residuals <- test_df$start_speed - test_df$predicted

ggplot(test_df, aes(x = predicted, y = residuals)) +
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, color = "red") +
  labs(title = "Residual Plot", x = "Predicted Pitch Speed", y = "Residuals")

```

```{r}
ggplot(test_df, aes(x = residuals)) +
...
```{r}
library(Metrics)
mae_val <- mae(test_df$start_speed, test_df$predicted)
rmse_val <- rmse(test_df$start_speed, test_df$predicted)

metrics_df <- data.frame(
  Metric = c("MAE", "RMSE"),
  Value = c(mae_val, rmse_val)
)
...
predicted_class <- predict(rf_cls, newdata = test_df, type = "class")
confusionMatrix(predicted_class, test_df$pitch_type)
...
importance_df <- data.frame(Feature = rownames(importance(rf_cls)),
                            Importance = importance(rf_cls)[,1])
